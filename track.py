...
def download_object_model():
    os.makedirs(DIR_MODELS, exist_ok=True)
    if not os.path.exists(OBJECT_MODEL_PATH):
        print(">> SYNCING ASSET: OBJ_DETECTOR_PRO...")
        import urllib.request
        try:
...
def draw_objects_on_image(image, object_result, filter_cans=True):
    if not object_result or not object_result.detections:
        return image
    h, w = image.shape[:2]
    object_color = (0, 255, 127)
    ...
    for detection in object_result.detections:
        ...
        if filter_cans:
            can_keywords = ['can', 'bottle', 'cup', 'mug', 'container']
            if not any(keyword in category_name for keyword in can_keywords):
                continue
        ...
        (label_w, label_h), baseline = cv2.getTextSize(label, FONT_FACE, FONT_SMALL, 1)
        label_y = max(y1 - int(5 * PADDING_SCALE), label_h + int(5 * PADDING_SCALE))
        cv2.rectangle(image, (x1, label_y - label_h - int(3 * PADDING_SCALE)), (x1 + label_w + int(4 * PADDING_SCALE), label_y + int(3 * PADDING_SCALE)), object_color, -1)
...
def show_startup_menu():
    ...
    print("   HAND + FACE + OBJECT TRACKER")
...
def show_profile_dialog():
    ...
    while True:
        screen = np.full((h, w, 3), DARK_BG, dtype=np.uint8)
        cv2.rectangle(screen, (0, 0), (w, h), PANEL_BG, -1)
        cv2.rectangle(screen, (20, 20), (w - 20, h - 20), CORE, 2)

        title = "SELECT TRACKING PROFILE"
        (tw, th), _ = cv2.getTextSize(title, FONT_FACE, 0.7, 2)
        cv2.putText(screen, title, ((w - tw) // 2, 70), FONT_FACE, 0.7, CORE, 2)
        cv2.putText(screen, "Press 1, 2, or 3 on your keyboard", ((w - 320) // 2, 110), FONT_FACE, 0.45, TEXT, 1)

        y0 = 150
        line_h = 42
        cv2.putText(screen, "[ 1 ]  FAST    — FPS optimized (staggered detection)", (50, y0), FONT_FACE, 0.45, TEXT, 1)
        cv2.putText(screen, "[ 2 ]  QUALITY — Visual max (concurrent landmarks)", (50, y0 + line_h), FONT_FACE, 0.45, TEXT, 1)
        cv2.putText(screen, "[ 3 ]  AUTO    — Architecture adaptive (recommended)", (50, y0 + 2 * line_h), FONT_FACE, 0.45, CORE, 1)

        cv2.putText(screen, "Waiting for key...", (50, h - 40), FONT_FACE, 0.4, TEXT, 1)
...
def main():
    ...
    if getattr(sys, 'frozen', False):
        fast_mode = show_profile_dialog()
    else:
        fast_mode = show_startup_menu()
    ...
    while True:
        ...
        for i, (stage_name, stage_progress) in enumerate(loading_stages):
            if progress >= stage_progress:
                current_stage_idx = i
        
        if current_stage_idx < len(loading_stages):
            status_text = loading_stages[current_stage_idx][0]
            stage_progress = loading_stages[current_stage_idx][1]
            if current_stage_idx > 0:
                prev_progress = loading_stages[current_stage_idx - 1][1]
                local_progress = (progress - prev_progress) / (stage_progress - prev_progress)
                display_progress = prev_progress + local_progress * (stage_progress - prev_progress)
            else:
                display_progress = progress / stage_progress * stage_progress
        else:
            status_text = "READY"
            display_progress = 1.0
        
        loading_screen = create_loading_screen(screen_width, screen_height, display_progress, status_text)
    ...
    try:
        while True:
            ...
            if frame_index % object_stagger == 1 or last_object_result is None:
            ...
            num_objects = len(object_result.detections) if object_result and object_result.detections else 0
            total_fingers = 0
            gestures = []
            ...
            if num_hands > 0 or num_faces > 0 or num_objects > 0:
                ...
                if not use_prev_graph and num_objects > 0:
                    display_lines.append(f"Objects: {num_objects}")
                if not use_prev_graph:
                ...
                graph_panel = np.full((graph_height, graph_width, 3), DARK_BG, dtype=np.uint8)
                no_hands_text = "NO HANDS, FACE, OR OBJECTS"
            ...
            if num_hands > 0 or num_faces > 0 or num_objects > 0:
                info_text = f"HANDS: {num_hands} | FINGERS: {total_fingers} | FACES: {num_faces} | OBJECTS: {num_objects}"
                ...
                if num_hands > 0 and detection_result.hand_landmarks and saved_gestures:
